...
diferente da livelo, o meliuz nao exige login para consultar o percentual de cashback de uma loja, entao esta consulta e tao simples quanto uma pessoa buscando a loja no site do meliuz antes de comprar.

sobre a esfera, ao pesquisar como o programa funciona, a lista de lojas parceiras so fica visivel depois de entrar com cpf e senha na conta. por esse motivo este projeto nao inclui um scraper de esfera, nao faz sentido automatizar uma consulta que exige login numa conta pessoal. os pontos esfera de uma oferta continuam sendo cadastrados manualmente na calculadora, do mesmo jeito que ja funcionava antes.

assim como no scraper da livelo, este modulo nao usa nenhuma tecnica agressiva de disfarce de navegador, apenas um user agent realista e a ocultacao do sinal mais comum de automacao. se o meliuz passar a bloquear essas consultas, a alternativa e cadastrar o cashback manualmente na calculadora, o campo ja existe para isso.
"""
...
class ErroScraperMeliuz(Exception):
    """
    erro especifico do scraper do meliuz, para diferenciar falha de rede ou de bloqueio de um erro generico de programacao.
    """
...
def gerar_slug_loja(nome_loja):
    """
    aproximacao do slug usado pelo meliuz para a pagina de uma loja, por exemplo "Fast Shop" vira "fast-shop". nem sempre bate com o slug real cadastrado pelo meliuz.
    """
...
def buscar_cashback_por_loja(nome_loja, timeout_ms=45000, headless=True, salvar_debug_em_falha=True):
    ...
    tenta primeiro o slug adivinhado a partir do nome da loja, e se a pagina nao trouxer nenhum percentual reconhecido, tenta a busca interna do site. sempre devolve um CashbackLoja, com encontrado=False quando nada foi localizado, em vez de levantar excecao, ja que a loja pode simplesmente nao ser parceira.
    """
