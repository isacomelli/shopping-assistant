...
def buscar_parceiro_para_loja(nome_loja, parceiros_conhecidos=None):
    """
    tenta encontrar um parceiro Livelo equivalente ao nome da loja
    encontrada no buscape.

    quando parceiros_conhecidos e informado, tipicamente pelo chamador
    de producao em routers/ofertas.py, que passa os parceiros ja
    atualizados na tabela livelo_parceiros do banco, misturados com o
    fallback de PARCEIROS_LIVELO_CONHECIDOS para os que ainda nao
    tiverem sido atualizados, a busca usa essa lista. quando nao e
    informado, None por padrao, cai para PARCEIROS_LIVELO_CONHECIDOS
    sozinho, o que mantem esta funcao e seus testes funcionando sem
    precisar de banco nenhum.
...
def montar_oferta_a_partir_do_buscape(oferta_buscape, cotacao_dolar,
    ...
    parceiros_conhecidos segue a mesma regra de buscar_parceiro_para_loja,
    None cai para PARCEIROS_LIVELO_CONHECIDOS.
    """
...
def pesquisar_produto_automaticamente(nome_produto, rendimento_mensal,
    ...
    parceiros_conhecidos e a lista de parceiros Livelo usada para casar
    cada loja encontrada, tipicamente montada por routers/ofertas.py
    juntando os parceiros ja atualizados na tabela livelo_parceiros do
    banco com o fallback fixo de PARCEIROS_LIVELO_CONHECIDOS para os
    que ainda nao tiverem sido atualizados. quando None, so o fallback
    fixo e usado, o que mantem esta funcao testavel sem depender de
    banco nenhum.
