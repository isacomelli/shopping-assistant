...
def rodar_livelo(termo, headless, salvar_banco=False):
    ...
    if salvar_banco:
        ...
        print(f"\n{len(parceiros)} parceiros gravados na tabela livelo_parceiros")
...
def main():
    ...
    if args.salvar_banco and args.scraper != "livelo":
        print("--salvar-banco so esta implementado para o livelo por enquanto")
        sys.exit(1)
    ...
    if args.reparsear:
        ...
        resultado = rodar_livelo(
            args.termo, headless=not args.mostrar_navegador, salvar_banco=args.salvar_banco,
        )
    else:
